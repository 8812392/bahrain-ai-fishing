...
def train_model(df: pd.DataFrame, target_col: str):
    ...
    if missing:
        raise ValueError(
            ...
            + "\n\nTip: If your CSV uses different names, rename them to match the required columns."
        )
    ...
    # Provide some safe defaults for UI option lists (from data)
    options = {
        "fishing_method": sorted(work["fishing_method"].dropna().astype(str).unique().tolist()),
        "target_species": sorted(work["target_species"].dropna().astype(str).unique().tolist()),
        "area_type": sorted(work["area_type"].dropna().astype(str).unique().tolist()),
        "gear_type": sorted(work["gear_type"].dropna().astype(str).unique().tolist()),
        "uses_bycatch_reduction_devices": sorted(
            work["uses_bycatch_reduction_devices"].dropna().astype(str).unique().tolist()
        ),
        "enforcement_level": sorted(work["enforcement_level"].dropna().astype(str).unique().tolist()),
        "status_of_target_species": sorted(work["status_of_target_species"].dropna().astype(str).unique().tolist()),
    }
